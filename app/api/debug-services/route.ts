...
export async function POST(req: NextRequest) {
  ...
  const { action, userId, serviceData } = body
  ...
  try {
    ...
    if (action === 'INSERT') {
      console.log('🔍 DEBUG: Inserting service:', serviceData)

      // Log the exact payload
      result.payloadSent = serviceData
      ...
      if (data && data[0]) {
        ...
        result.insertedColumns = Object.keys(data[0])
      ...
      console.log('🔍 DEBUG: Selecting services for user:', userId)
